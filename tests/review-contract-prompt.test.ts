...
test("before_agent_start injects the mirrored review execution contract for the primary session", async () => {
	...
	assert.match(result.systemPrompt, /RDD switch is (?:the )?complete (?:review )?authorization/i);
	...
	assert.doesNotMatch(result.systemPrompt, /gentle-ai\.review-integration\.consent\/v3|answer-consent/);
});
