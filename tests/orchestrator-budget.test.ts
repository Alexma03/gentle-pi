...
const SUPERSEDED_LIFECYCLE_REVIEW_LINES = new Set([
	70,
	// 74/77: the loose mode-choice lines were replaced by the canonical
	// delegation contract (issue #256).
	74,
	76,
	77,
	92,
...
test("core-alone: dynamic Gentle AI ownership replaces package lifecycle instructions", () => {
	const core = readRealAsset("orchestrator.md");
	assert.match(core, /injects the verified mirrored provider-bundle review execution contract into the primary session's system prompt at start/);
