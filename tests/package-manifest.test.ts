...
test("generated runtime modules and packed-package checks are deterministic", () => {
	...
	assert.equal(ci.match(/pnpm run install:gentle-ai-fork/g)?.length, 1);
	assert.match(ci, /GENTLE_PI_REQUIRE_DEV_BINARY: "1"/);
	assert.match(ci, /process\.platform==='win32'\?'gentle-ai\.exe':'gentle-ai'/);
	assert.match(ci, /GENTLE_AI_DEV_BINARY='\+binary/);
	assert.match(ci, /GENTLE_PI_GENTLE_AI_DEV_BINARY='\+binary/);
	assert.doesNotMatch(ci, /GENTLE_PI_REQUIRE_NATIVE_BINARY/, "fork CI must not demand the separate published-release fixture");
	assert.doesNotMatch(ci, /pnpm run check:release-evidence/, "aggregate tests already execute the release-evidence check once");
	assert.match(generator, /Generated by scripts\/build-runtime-modules\.mjs/);
	assert.match(packedRunner, /\["install"[^\]]*"--ignore-scripts"/s, "packed install must reuse the single verified fork build");
	assert.match(packedRunner, /GENTLE_PI_CONFIG_HOME: isolatedConfigHome/);
	assert.match(packedRunner, /assertAmbientRegistrationsUnchanged/);
...
test("package publication runs the release evidence gate before package operations", () => {
	...
	const installer = "pnpm run install:gentle-ai-fork";
	...
	assert.ok(prepublishOnly.indexOf(releaseGate) < prepublishOnly.indexOf(installer), "release evidence must fail closed before a publication build");
...
test("v2.3.0-rc.1 release package and runtime stop before publication", () => {
	...
	assert.equal(
		...
		"pnpm run test:unit && pnpm run test:contracts && pnpm run test:harness",
	);
	assert.equal(packageJson.scripts?.["test:unit"], "node --import ./scripts/canonical-test-temp.mjs --experimental-strip-types --test tests/*.test.ts");
	assert.equal(packageJson.scripts?.["test:contracts"], "pnpm run check:provider-contract && pnpm run check:runtime-modules && node scripts/verify-package-files.mjs && pnpm run check:release-evidence");
	assert.equal(
